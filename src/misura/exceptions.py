...
class UnpackError(Exception):
    ...
    def __init__(self, qnt, target: str) -> None:
        super().__init__("cannot unpack \'{1}\' from \'{0}\'\nraised by: \'{2}\'".format(qnt.unit(), target, qnt))
...
class PackError(Exception):
    ...
    def __init__(self, qnt, target: str, full: bool = False) -> None:
        super().__init__("cannot {3}pack \'{0}\' to \'{1}\'\nraised by: \'{2}\'".format(qnt.unit(), target, qnt, "fully " if full else "") if target != "" else "cannot automatically pack")
